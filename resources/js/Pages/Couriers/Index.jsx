...
const Couriers = (props) => {
  ...
  const rows = couriers.map((courier, index) => (
    <Table.Tr key={courier.id} style={{ height: '48px' }}>
      <Table.Td>{index + 1}</Table.Td>
      <Table.Td>{courier.name}</Table.Td>
      <Table.Td>{courier.phone_number}</Table.Td>
      <Table.Td>{courier.address}</Table.Td>
      ...
      <Table.Td>
        <div style={{ display: 'flex', gap: '8px' }}>
          <ActionIcon
            ...
            color="yellow"
            onClick={() =>
              router.get(route('couriers.edit', { id: courier.id }))
            }
          >
            <IconEdit />
          </ActionIcon>
          <ActionIcon
            ...
            color="red"
            onClick={() =>
              router.delete(route('couriers.destroy', { id: courier.id }))
            }
          >
            <IconTrash />
          </ActionIcon>
  ...
  return (
    ...
    >
      <Group justify="space-between">
        ...
        <Button onClick={() => router.get(route('couriers.create'))}>
          Tambah Kurir
        </Button>
      </Group>
